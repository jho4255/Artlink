...
- `frontend/src/__tests__/missingNote.test.ts` 4개 — 누락 메모가 목록을 자르지 않는지 포함
- E2E `e2e/tests/28-download-recovery.spec.ts` 5개 — 자동 회수, 배너 지속(토스트 소멸 시간 경과 후에도), ZIP 자기기록, **정산서·지원서 PDF(그동안 커버리지 0)**
