...
- `GET /api/explore/highlight?limit=8` (인증 불필요) → `{ images, basis }`
  - `week`(최근 7일 좋아요 ≥1) → "이번 주 인기 작품" / `all` → "많이 사랑받은 작품" / `random`(좋아요 전무, **날짜 시드**로 하루 고정) → "작가들의 작품"
  - 초기에는 주간 집계가 전부 0이라 클라이언트가 분기하면 API를 두 번 부르게 된다 → **서버가 폴백까지 판단**한다.
- `components/home/ExploreHighlight.tsx` — HomePage의 GalleryOfMonth 아래. 부제 **"여러분의 좋아요로 뽑혔습니다"** 로 인과를 노출해 좋아요를 큐레이션 참여로 만든다.
