...
describe('작품 크기 문자열 읽기 — 문자열은 세로×가로, 반환은 [가로, 세로]', () => {
  // ⚠️ 2026-09-16 부터 크기 문자열은 **세로×가로**(관례)다. "116.8 × 91.0 cm" 는 세로 116.8·가로 91 이므로
  //    [가로, 세로] = [91, 116.8]. `lib/artwork.ts composeSize`·`parseAspect` 와 같은 순서를 읽는지 여기서 잠근다.
  it.each([
    ['116.8 × 91.0 cm', [91, 116.8]],
    ['80x60', [60, 80]],
    ['80×60cm', [60, 80]],
    ['90.9 X 72.7', [72.7, 90.9]],
    ['30호 (90.9×72.7cm)', [72.7, 90.9]],
    ['1168 × 910 mm', [91, 116.8]],   // mm 표기는 cm 로 내린다
  ])('%s → %s', (text, expected) => {
