...
24. **`frontend/index.html`의 SEO 마커 삭제 금지** — `<!--SEO_META_START-->`~`<!--SEO_META_END-->` 사이를 서버(`lib/seoMeta.ts`)가 상세 페이지 요청 시 교체한다. 마커가 사라지면 **에러 없이 조용히** 기본 meta로 되돌아감(카톡 공유 미리보기·검색 노출 전부 무력화). meta 값을 코드로 만들 때는 반드시 `buildMetaTags()`를 통과시킬 것 — 직접 문자열 연결 시 XSS. 킬스위치 `SEO_META=off`
25. **정산 재확인은 `lib/settlementFingerprint.ts` 로만 판정** — 작가가 응답할 때 본 금액을 `SettlementApproval.snapshot` 에 지문으로 남기고, 저장할 때 대조해 **금액이 바뀐 작가만** 재확인 대상으로 되돌린다(그 사람에게만 알림). 지문에 `ArtworkSale.title` 을 넣지 말 것 — 화면 제목은 작가 출품목록에서 오므로, 넣으면 금액이 그대로인데도 멀쩡한 수락이 풀린다. `findMany` 순서를 믿지 말고 반드시 index 정렬 후 생성.
...
26. **`truncate` 는 min-content 를 줄이지 않는다** — `overflow:hidden` 은 flex 자동 최소치의 '바닥'만 없앨 뿐이다. min-content 로 트랙 폭이 정해지는 grid 안에서는 nowrap 텍스트가 폭을 그대로 밀어내므로, **grid 아이템마다 `min-w-0` 을 빠짐없이** 붙일 것. 하나만 빼먹어도 375px 에서 페이지 전체가 가로로 밀린다(실측 447px). 상태 배지처럼 짧은 칩은 `shrink-0 whitespace-nowrap` 이 없으면 '대 기 중' 으로 세로로 쪼개진다
