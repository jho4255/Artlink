...
LIMITS = {
    ...
    # ⚠️ 판정은 **비율**(dir_rel)로. 골든 5장에 어두운 액자가 없어 절대 임계는 재질
    #    페널티가 된다(검정 살 46 · 월넛 60 은 옳게 그늘져도 절대 낙차 30 을 못 넘는다).
    #    골든 상대 실측 0.146 / 0.180 / 0.146 / 0.157 / 0.747.
    'dir_rel':       ('range', (0.13, 0.85),          '0.15~0.75', '위살↔아래살 밝기차 ÷ 살 밝기 = 방향광'),
...
def ok(name, v, g):
    ...
    if name == 'rebate_ratio' and g.get('frameKind') == 'canvas':
        return True
    # ⚠️ 캔버스 랩은 '살'이 없다 — 옆면이 **그림이 감긴 면**이라 그 밝기는 액자 조명이
    #    아니라 **작품의 그 자리 색**이다. 위↔아래 차이를 재면 그림 내용을 재는 셈이라
    #    값이 작품마다 널뛴다(실측 0.8~0.9). 임계 완화가 아니라 **해당 없음**.
    if name == 'dir_rel' and g.get('frameKind') == 'canvas':
        return True
    if kind == '<=':
        return v <= lim
    if kind == '>=':
