...
## 2026-09-16 — 지면 체계 v2 (CLAUDE.md 54)

- `combos.mjs` 를 다시 돌리기 전에 `combos-before/` 로 이전 PDF 를 복사해 두면 전/후 비교가 된다(이번엔 20권 전부 보관).
- `emptypages.py combos/*.pdf` — 거의 빈 장 비율. v2 전 20권 390쪽 중 13.1% → 후 1~9권 2.0%(남은 것은 사진 없는 작가의 닫는 장).
- 골든 5권(`../../portfolio/`)과 **같은 높이로 나란히** 놓는 비교 시트가 비평의 시작이었다 — 지표가 다 통과해도 눈에 지는 지점(자신감·가운데 쌓기·
  기준선·강조색·행간)은 이렇게만 보인다. 새 배치를 만들면 반드시 골든 옆에 놓고 볼 것.
- 벡터 PDF 검증은 `e2e/_pfprint.mjs`(page.pdf), 버전 UI 왕복은 `e2e/_pfversions.mjs`.
