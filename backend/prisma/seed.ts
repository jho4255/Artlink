...
async function main() {
  ...
  const gallery1 = await prisma.gallery.upsert({
    where: { id: 1 },
    update: { instagramUrl: '@seoul_modern_gallery', instagramProfileVisible: true, email: 'info@seoulmodern.com' },
    create: {
      name: '서울 현대 갤러리',
  ...
  const gallery2 = await prisma.gallery.upsert({
    where: { id: 2 },
    update: { instagramUrl: '@busan_art_center', instagramProfileVisible: true, email: 'contact@busanart.kr' },
    create: {
      name: '부산 해운대 아트센터',
